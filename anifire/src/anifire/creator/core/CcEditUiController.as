...
package anifire.creator.core
{
   ...
   public class CcEditUiController extends EventDispatcher
   {
      ...
      public function start(param1:CcCharacter, charCopyMode:Boolean) : void
      {
         this.ui_ce_container.ui_ce_bodyShapeChooser.init(this.currentTheme, charCopyMode);
      ...
      private function switchComponentType(type:String, param2:Boolean) : void
      {
         ...
         if(param2)
         {
            this.ui_ce_container.ui_ce_componentTypeChooser.switchToComponentType(type,false);
         }
         ...
         if(type != CcLibConstant.COMPONENT_TYPE_BODYSHAPE)
         {
            if(type == CcLibConstant.COMPONENT_GROUP_UPPER_LOWER)
            {
               ...
               this.ui_ce_container.ui_ce_clothesChooser.init(this.ccChar,this.currentTheme,type,false);
               this.ui_ce_container.ui_ce_thumbPropertyInspector.destroy();
            }
            else
            {
               _loc5_.push(type);
               this.ui_ce_container.ui_ce_componentThumbChooser.init(this.ccChar,this.currentTheme,type,CcLibConstant.ALL_MULTIPLE_COMPONENT_TYPES.indexOf(type) > -1 ? false : true);
               this.ui_ce_container.ui_ce_thumbPropertyInspector.destroy();
            }
         }
         this.ui_ce_container.ui_ce_colorPicker.destroy();
