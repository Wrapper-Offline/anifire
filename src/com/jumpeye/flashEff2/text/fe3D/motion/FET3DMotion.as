package com.jumpeye.flashEff2.text.fe3D.motion
{
   ...
   public class FET3DMotion extends IFlashEffText
   {
      ...
      protected function startTransition(param1:Boolean) : void
      {
         ...
         this.elements = [];
         ow = Number(this.component.targetOwner.width);
         oh = Number(this.component.targetOwner.height);
         ...
         while(i < lin)
         {
            ...
            while(j < col)
            {
               ...
               x = Number(elementObject.clip.x);
               y = Number(elementObject.clip.y);
               w = Number(elementObject.clip.width);
               h = Number(elementObject.clip.height);
               z = this.radius;
