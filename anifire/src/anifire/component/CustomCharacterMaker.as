package anifire.component
{
	...
	public class CustomCharacterMaker extends Sprite implements ICharacter
	{
		...
		private function getSkeletonContainer(ver:Number = 0) : DisplayObjectContainer
		{
			var container:DisplayObjectContainer;
			switch (ver)
			{
				case 2:
					container = UtilPlain.getInstance(this, CcLibConstant.COMPONENT_TYPE_FREEACTION);
					break;
				default:
					container = UtilPlain.getInstance(this, CcLibConstant.COMPONENT_TYPE_SKELETON);
			}
			return container;
		}
		...
		/**
		 * adds libraries and colors from the action
		 */
		public function prepareDataByCam() : void
		{
			...
			{
				var libraryPath:String = this._myActionModel.libraryPaths[i] as String;
				this.addLibrary(i, libraryPath, this._myActionModel.themeId);
			}
			for (i in this._myActionModel.colorCodes)
			{
				var color:CCColor = this._myActionModel.colorCodes[i];
				this.addColor(color.type, color.oc, color.dest);
			}
		}

		/**
		 * adds libraries and colors from the xml
		 */
		private function prepareXML(charXml:XML) : void
		{
			this._ver = String(charXml.@version).length > 0 ? Number(charXml.@version) : 1;
			var i:int = 0;
			var elements:XMLList = charXml.library;
			for (; i < elements.length(); i++)
			{
				this.addLibrary(elements[i].@type, elements[i].@component_id, elements[i].@theme_id);
			}
			i = 0;
			elements = charXml.color;
			for (; i < elements.length(); i++)
			{
				this.addColor(elements[i].@r, uint(elements[i].@oc.length() > 0 ? elements[i].@oc : uint.MAX_VALUE), uint(elements[i].text()));
			}
		}
		
		public function removeLibrary(type:String) : void
		{
			this._libraries.removeByKey(type);
		}
		
		public function addLibrary(type:String, id:String, themeId:String) : void
		{
			var library:Library = new Library();
			library.type = type;
			library.component_id = id;
			library.theme_id = themeId;
		...
		private function requestImage() : void
		{
			...
			if (this._useImageLibrary)
			{
				...
				if (this._myActionModel)
				{
					...
					{
						_loc2_ = new ExtraDataLoader();
						_loc7_ = String(this._myActionModel.libraryPaths[_loc12_]);
						_loc9_ = 0;
						if ((_loc9_ = CcImageLibrary.library.requestImage(_loc7_, this._sceneId, _loc2_)) <= 0)
						{
							if (this.charZip != null)
		...
		public function initByCamWithData(cam:CCCharacterActionModel, startMs:Number = 0, endMs:Number = 0, imageData:UtilHashBytes = null, isSpeech:Boolean = false, unused:Boolean = false, filename:String = "") : void
		{
			if (!cam)
			{
				return;
			}
			var actionLoader:CcActionLoader = CcActionLoader.getActionLoader(filename);
			actionLoader.addEventListener(Event.COMPLETE, this.onCcActionLoadedByCam);
			actionLoader.addEventListener(IOErrorEvent.IO_ERROR, this.onCcActionFailed);
		...
		public function updateComponentImageData(componentType:String, swfByteArray:ByteArray, properties:Object, loadMgr:UtilLoadMgr, colors:Array = null, id:String = "", node:XML = null) : ExtraDataLoader
		{
			...
			if (this._useImageLibrary)
			{
				...
				if (this._myActionModel)
				{
					key = String(this._myActionModel.getComponentByType(componentType).path);
			...
			if (swfByteArray == null)
			{
				...
				{
					swfkey = String(this._myActionModel.getComponentByType(componentType).path);
				}
				if (this.charZip != null)
				{
			...
			if (this._useImageLibrary)
			{
				...
				if (this._myActionModel)
				{
					for (_loc9_ in this._myActionModel.components)
					{
						...
						{
							_loc7_ = String(this._myActionModel.getComponentByType(_loc9_).path);
							if (!param2)
							{
								_loc11_ = this._assetImageIdArray.getValueByKey(_loc7_);
							}
