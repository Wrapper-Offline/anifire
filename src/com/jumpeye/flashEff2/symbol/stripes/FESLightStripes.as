package com.jumpeye.flashEff2.symbol.stripes
{
   ...
   public class FESLightStripes extends IFlashEffSymbol
   {
      ...
      {
         var _loc2_:Number = Number(param1.prop);
      ...
      protected function startTransition(param1:Boolean = true) : void
      {
         ...
         var dynamicPos:String;
         var twoPixels:uint;
         var currentPos:Number;
         var dim1:String;
         var targetBmpContainer:Sprite;
         var dim2:String;
         var targetBmp:Bitmap;
         var targetDim:Number;
         var stripesNr:Number;
         var startMatrix:Matrix;
         var stripeDelay:Number;
         var scale:String;
         var preset:Number;
         var targetDim1:Number;
         var start:uint;
         var i:uint;
         var targetDim2:Number;
         var end:uint;
         ...
         targetBD = new BitmapData(int(this.target.width) || 1,int(this.target.height) || 1,true,0);
         targetBD.draw(this.target,startMatrix);
         targetBmp = new Bitmap(targetBD);
         targetBmpContainer = new Sprite();
         ...
         fixPos = "x";
         dynamicPos = "y";
         targetDim1 = this.target.width;
         targetDim2 = this.target.height;
         start = 0;
         end = 1;
         if(show == false)
         {
         ...
         targetDim = Math.ceil(targetDim2);
         currentPos = Math.ceil(targetDim1);
         switch(this.preset)
         {
            case 2:
         ...
         stripeDelay = Math.max((this.tweenDuration - this.groupDuration) / (stripesNr - 1),0);
         i = stripesRef.length;
