	...
	public dynamic class FlashEff2 extends JUIComponent
	{
		...
		public function drawTextTable(param1:String, param2:String, param3:Number, param4:Array, param5:Number, param6:Number, param7:Array = null) : Boolean
		{
			...
			switch(param1.toLocaleLowerCase())
			{
				...
				case "words":
					...
					while(_loc14_ < _loc17_)
					{
						_loc31_ = Number(_loc10_[_loc14_].owner.x);
						_loc32_ = Number(_loc10_[_loc14_].owner.y);
						...
						while(_loc15_ < _loc18_)
						{
							...
							while(_loc16_ < _loc19_)
							{
								...
								if(_loc11_[_loc16_].passed == false)
								{
									...
									{
										_loc33_ = Number(_loc8_.x);
										_loc34_ = Number(_loc8_.y);
									}
								}
								_loc30_ = _loc29_.pushChild({
									"type":"FeChar",
								...
								_loc20_ = uint(this.textField.getLineIndexOfChar(_loc30_.id));
			...
			while(_loc14_ < _loc25_)
			{
				...
				if((_loc20_ = uint(this.textField.getLineIndexOfChar(_loc30_.id))) != _loc26_)
				{
					_loc26_ = _loc20_;
		...
		protected function filterArray(param1:Array, param2:String, param3:Number, param4:Array) : void
		{
			...
			else
			{
				...
				{
					_loc13_ = 0;
					_loc14_ = String(param4[_loc8_]);
					_loc15_ = _loc11_.indexOf(_loc14_,_loc13_);
					_loc16_ = _loc14_.length;
					_loc17_ = / /g;
					_loc19_ = uint((_loc18_ = _loc14_.replace(_loc17_,"")).length);
					while(_loc15_ != -1)
					{
						_loc20_ = _loc15_;
		...
		protected function wordsSplit(param1:String, param2:uint) : Array
		{
			...
			while(_loc8_ < _loc5_)
			{
				...
				{
					_loc10_ = true;
					if((_loc11_ = uint(_loc3_[_loc8_].length)) > 0)
					{
						_loc12_ = 0;
		...
		protected function linesSplit() : Array
		{
			...
			var _loc1_:Array = [];
			var _loc2_:Array = [];
			var _loc3_:uint = uint(this.textField.numLines);
			var _loc4_:uint = 0;
			var _loc5_:Number = 0;
			var _loc6_:uint = 0;
			while(_loc6_ < _loc3_)
			{
				...
				{
					_loc9_ = true;
					if((_loc10_ = uint(_loc8_.length)) > 0)
					{
						_loc11_ = 0;
		...
		protected function customSplit(param1:String, param2:uint, param3:Array) : Array
		{
			...
			while(_loc8_ < _loc5_)
			{
				...
				{
					_loc11_ = true;
					if((_loc12_ = uint(_loc9_.length)) > 0)
					{
						_loc13_ = 0;
		...
		protected function setProp(param1:XMLList, param2:Object) : void
		{
			...
			if(refObj != null)
			{
				for each(child in arg.children())
				{
					...
					propName = String(child.name().localName);
					try
					{
						switch(propType)
						{
		...
		internal function setDrawTime() : void
		{
