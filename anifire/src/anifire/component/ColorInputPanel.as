package anifire.component
{
	...
	public class ColorInputPanel extends SkinnableComponent
	{
		...
		/**
		 * called when the user presses their mouse over the color field,
		 * begins listening for mouseMove and mouseUp events across the stage
		 */
		protected function colorFieldGroup_mouseDownHandler(event:MouseEvent) : void
		{
			this._draggingColorField = true;
			this.colorFieldGroup.stage.addEventListener(MouseEvent.MOUSE_MOVE, this.colorFieldGroup_mouseMoveHandler);
			this.colorFieldGroup.stage.addEventListener(MouseEvent.MOUSE_UP, this.colorFieldGroup_mouseUpHandler);
			this.handleMousePosition(this.colorFieldGroup.mouseX, this.colorFieldGroup.mouseY);
		}
		
		protected function colorFieldGroup_mouseMoveHandler(event:MouseEvent) : void
		{
			this.handleMousePosition(this.colorFieldGroup.mouseX, this.colorFieldGroup.mouseY);
		}
		 ...
		 */
		protected function colorFieldGroup_mouseUpHandler(event:MouseEvent) : void
		{
			if (this._draggingColorField) {
				this.colorFieldGroup.stage.removeEventListener(MouseEvent.MOUSE_MOVE, this.colorFieldGroup_mouseMoveHandler);
				this.colorFieldGroup.stage.removeEventListener(MouseEvent.MOUSE_UP, this.colorFieldGroup_mouseUpHandler);
				this.notifyColorChange();
		...
		/**
		 * translates the mouse's x and y positions into saturation
		 * and brightness values, and updates the hex field accordingly
		 */
		protected function handleMousePosition(mouseX:int, mouseY:int) : void
		{
			if (mouseX < 0) {
				mouseX = 0;
			} else if(mouseX > COLOR_FIELD_SIZE) {
				mouseX = COLOR_FIELD_SIZE;
			}
			if(mouseY < 0) {
				mouseY = 0;
			} else if(mouseY > COLOR_FIELD_SIZE) {
				mouseY = COLOR_FIELD_SIZE;
			}
			this.saturation = mouseX / COLOR_FIELD_SIZE;
			this.brightness = 1 - mouseY / COLOR_FIELD_SIZE;
			this.updateHexColorSpace();
		}
		...
		protected function eyeDropperScreenCap_eyeDropperCompletedHandler(event:EyeDropperEvent) : void
		{
			var dropper:EyeDropperScreenOverlay = anifire.component.EyeDropperScreenOverlay.instance;
			if (!dropper) {
				return;
			}
			dropper.removeEventListener(EyeDropperEvent.EYE_DROPPER_COMPLETE, this.eyeDropperScreenCap_eyeDropperCompletedHandler);
			dropper.removeEventListener(EyeDropperEvent.EYE_DROPPER_CANCEL, this.eyeDropperScreenCap_eyeDropperCancelHandler);
			this.currentColor = event.color;
			this.notifyColorChange();
			dispatchEvent(event.clone());
		}
		
		protected function eyeDropperScreenCap_eyeDropperCancelHandler(event:EyeDropperEvent) : void
		{
			var dropper:anifire.component.EyeDropperScreenOverlay = anifire.component.EyeDropperScreenOverlay.instance;
			if (!dropper) {
				return;
			}
			dropper.removeEventListener(EyeDropperEvent.EYE_DROPPER_COMPLETE, this.eyeDropperScreenCap_eyeDropperCompletedHandler);
		...
		/**
		 * dispatches an event signifying the color has been fully
		 * selected and is ready to be committed as a change
		 */
		protected function notifyColorChange() : void
		{
			var event:ColorPaletteEvent = new ColorPaletteEvent(ColorPaletteEvent.COLOR_CHANGE);
			event.colorCode = this.currentColor;
			dispatchEvent(event);
		}

		/**
		 * dispatches an event signifying the color needs to be displayed
		 * on the colorDisplay component
		 */
		protected function notifyColorPreview() : void
		{
			var event:ColorPaletteEvent = new ColorPaletteEvent(ColorPaletteEvent.COLOR_PREVIEW);
			event.colorCode = this.currentColor;
			dispatchEvent(event);
		}
	}
